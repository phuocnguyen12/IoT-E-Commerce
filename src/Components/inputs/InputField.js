  ...
  return (
    <div className={clsx('flex flex-col mb-2', fullWidth && 'w-full')}>
      ...
      <input
        type={type || 'text'}
        className={clsx(
          'px-4 py-2 rounded-sm border w-full mt-2 placeholder:text-sm placeholder:italic outline-none text-black',
          style
        )}
      ...
      {invalidFields?.some((el) => el.name === nameKey) && (
        <small className='text-main italic'>{invalidFields.find((el) => el.name === nameKey)?.mes}</small>
      )}
    </div>
  )
}
